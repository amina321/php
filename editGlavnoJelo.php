...
<input type="hidden" name="idglavno_jelo" value="<?php echo $idglavno_jelo; ?>"/>
...
if (is_numeric($_POST['idglavno_jelo']))
{

$idglavno_jelo = $_POST['idglavno_jelo'];
...
mysqli_query($conn, "UPDATE glavno_jelo SET naziv='$naziv', gramaza='$gramaza' ,cijena='$cijena' WHERE idglavno_jelo='$idglavno_jelo'")
...
if (isset($_GET['idglavno_jelo']) && is_numeric($_GET['idglavno_jelo']) && $_GET['idglavno_jelo'] > 0)
{

$idglavno_jelo = $_GET['idglavno_jelo'];
$result = mysqli_query($conn, "SELECT * FROM glavno_jelo WHERE idglavno_jelo=$idglavno_jelo")
or die(mysqli_error($conn));
...
valid($idglavno_jelo, $naziv, $gramaza,$cijena,'');
}
else
{
echo "No results!";
